...
class ProfileController extends Controller
{
    ...
    private function buildPortfolioPdf(User $user, array $portfolio): string
    {
        ...
        if (! empty($portfolio['stats']['has_soft_skill_certificate'])) {
            $noteY = max($y - 24, 70);
            $current .= "0.93 0.97 0.94 rg\n40 {$noteY} 515 18 re f\n";
            $current .= "BT\n0.13 0.36 0.21 rg\n/F1 9 Tf\n45 " . ($noteY + 6) . " Td\n(" . $this->escapePdfText('Soft Skill Certificate Status: Qualified') . ") Tj\nET\n";
        }
